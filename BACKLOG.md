...
| ID | Story | Status |
|---|---|---|
| #127 R1 | `cloudless join <link>` — accept the full join link/URL directly (as generated by E2), no manual secret@host:port assembly | ✅ (accepts the console/`up`-printed link verbatim — quoted whole or unquoted — strips a leading `cloudless up`, validates a `-join` flag is present, then reuses `up`'s existing flag parsing; verified against a real seed + joiner node pair, "✓ connected") |
...
| #132 R6 | Guided interactive first-run — `cloudless up` with no flags and no existing config prompts step-by-step instead of assuming the operator already knows the right flags | ✅ (only kicks in for zero-args + a real terminal + no existing config, so scripts/CI piping stdin are unaffected; asks join-vs-start, and only asks for a backend URL if auto-detection found nothing; verified with 6 unit tests plus real runs — flags still bypass it, piped zero-arg stdin still starts immediately with old defaults, no hang) |
