...
**Positioning:** the parent product is **Cloudless** — an alternative to commercial cloud: a community mesh network spread across continents with no data centres, running on commodity hardware and the systems people already own. **Cloud AI** (this MVP) is its first *child service*; Cloud Storage, Cloud Compute, and Cloud Data are later children riding the same mesh, security backbone, and console.
...
## Status log
- **M1b core BUILT ✅ (2026-07-18).** Cluster PKI: first `up` mints a CA and self-issues (A1); joiners enroll their public key with the seed, authenticated by HMAC over the cluster secret — the secret never travels, and a tampered key fails verification (A2-lite; single-use expiring tokens still open). Every node runs a mutual-TLS relay (TLS 1.3, CA-verified client certs); peer inference traffic flows only through it, advertised via gossip as `https://` relay URLs (A3). Verified: cross-node failover through the encrypted relay, certless clients rejected at handshake, forged enrollment rejected. Known v0.1 limits: enrollment rides the seed's plain HTTP port (response is public data; MITM-on-LAN hardening lands with the overlay), and revocation (A4) is next.
